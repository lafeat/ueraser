...
class LinearNet(nn.Module):
    def __init__(self, layers, indim, width, outdim, acti=nn.ReLU):
...
class TransposeBasicBlock(nn.Module):
    ...
    def __init__(self, in_planes, planes, stride=1):
        ...
        if(stride == 2):
            ...
            output_padding = 0
        self.conv1 = nn.ConvTranspose2d(in_planes, planes, kernel_size=3, stride=stride, padding=1, output_padding=output_padding, bias=False)
        self.bn1 = nn.BatchNorm2d(planes)
...
class ResNet(nn.Module):
    ...
    def _make_inverse_layer(self, block, in_planes, planes, num_blocks, stride):
        strides = [stride] + [1] * (num_blocks - 1)
        layers = []
        for i, stride in enumerate(strides):
            if(i != num_blocks - 1):
                ...
                layers.append(block(planes, 3, stride))
